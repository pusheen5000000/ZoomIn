...
**Detection** is a scikit-learn classifier trained on the **Yamana** Apache-2.0 TSV, plus BeautifulSoup/regex heuristics. Mathur 2019 is the **taxonomy we cite**; their GPLv3 CSVs are **not** in this repo. Groq is **not** the dark-pattern judge.
...
| | |
| --- | --- |
| URL | **http://localhost:5173** (use this origin, not `:8000`) |
...
Open **http://localhost:5173**. Vite proxies `/auth`, `/scan`, `/demo`, `/health`, `/subscriptions` to port 8000. Logging in on `:8000` directly will not set the cookie the UI expects.
...
- We are not lawyers. License table is our working stance.

---

## Licenses

We are not lawyers. This is what we ship vs cite.

**In the repo**

| Artifact | License | Where |
| --- | --- | --- |
| Yamana `ec-darkpattern` TSV | Apache-2.0 | `backend/classifier/data/` + `NOTICE.txt` |
| axe-core 4.10.3 | [MPL-2.0](https://www.mozilla.org/en-US/MPL/2.0/) | `backend/detect/vendor/` unmodified + `LICENSE-axe-core.txt` |
| Our app code | Hackathon / project | `backend/`, `frontend/` |

**Cite only — do not copy into git**

| Source | Why |
| --- | --- |
| [Mathur et al. 2019](https://arxiv.org/abs/1907.07032) / [GPLv3 repo](https://github.com/aruneshmathur/dark-patterns) | Paper + taxonomy only. No crawler/CSVs |
| WAVE / WebAIM dumps | [WAVE terms](https://wave.webaim.org/terms) |
| AccessGuru / DPDGPT | CC BY 4.0 would allow credit; we did not pull them |

Steel, Groq, Anthropic, and Safe Browsing are **network APIs**. Keys in `.env`. A scan keeps a **short text preview**, not a full mirror of someone else’s site.
...
6. **Deque.** axe-core v4.10.3. [github.com/dequelabs/axe-core](https://github.com/dequelabs/axe-core) (MPL-2.0).

7. **Pedregosa, F., et al.** (2011). *Scikit-learn: Machine Learning in Python*. JMLR 12.

8. **Beautiful Soup** — HTML parser for heuristics.

9. **Steel.dev** — [docs.steel.dev](https://docs.steel.dev). **Browser**, not Computer.

10. **Playwright** — [playwright.dev](https://playwright.dev). CDP into Steel.

11. **Google Safe Browsing** — [developers.google.com/safe-browsing](https://developers.google.com/safe-browsing).

12. **Groq** — [groq.com](https://groq.com) (`OPENAI_BASE_URL=https://api.groq.com/openai/v1`).

13. **Anthropic** — [docs.anthropic.com](https://docs.anthropic.com).

14. **FastAPI**, Uvicorn, Pydantic, httpx, python-dotenv.

15. **React**, **Vite**.

16. **Browser Use** — leftover. [github.com/browser-use/browser-use](https://github.com/browser-use/browser-use).
